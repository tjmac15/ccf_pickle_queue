...
export async function autoFillStaging(courtId) {
  // Read then sort locally instead of using a compound Firestore query.
  // This keeps Auto-fill working on projects that have not created the
  // optional `status + joinedAt` composite index yet.
  const playersSnap = await getDocs(collection(db, "players"));
  const allWaitingIds = playersSnap.docs
  ...
  const courtRefs = courtsSnap.docs.map((snap) => snap.ref);
  const candidateRefs = allWaitingIds.map((id) => doc(db, "players", id));

  // Read every active court in the same transaction before writing. Two
  // organizers cannot reserve the same player for different courts: the
  // losing transaction retries against the newest staging lists.
  await runTransaction(db, async (tx) => {
    const [settingsSnap, courtSnaps, candidateSnaps] = await Promise.all([
      tx.get(SETTINGS_REF()),
      Promise.all(courtRefs.map((ref) => tx.get(ref))),
      Promise.all(candidateRefs.map((ref) => tx.get(ref))),
    ]);
    const activeCourtCount = settingsSnap.data()?.courtCount || DEFAULT_SETTINGS.courtCount;
    const targetSnap = courtSnaps.find((snap) => snap.id === courtId);
    if (!targetSnap) throw new Error("court-not-found");

    const waitingIds = new Set(
      candidateSnaps
        .filter((snap) => snap.exists() && snap.data().status === "waiting")
        .map((snap) => snap.id)
    );
    // A player who has left the queue must not leave a hidden, unusable
    // slot behind in Up Next.
    const thisStaged = (targetSnap.data().staged || []).filter((id) => waitingIds.has(id));
    const stagedElsewhere = new Set();
    courtSnaps.forEach((snap) => {
      const court = snap.data();
      if ((court.number || Number(snap.id)) > activeCourtCount || snap.id === courtId) return;
      (court.staged || []).forEach((id) => stagedElsewhere.add(id));
    });

    const available = allWaitingIds.filter(
      (id) => waitingIds.has(id)
    ).filter(
      (id) => !stagedElsewhere.has(id) && !thisStaged.includes(id)
    );
    const needed = Math.max(0, 4 - thisStaged.length);
    tx.update(targetSnap.ref, { staged: [...thisStaged, ...available.slice(0, needed)] });
  });
}
...
export async function addToStaging(courtId, playerId) {
  ...
  const courtRefs = courtsSnap.docs.map((snap) => snap.ref);
  await runTransaction(db, async (tx) => {
    const [settingsSnap, courtSnaps, playerSnap] = await Promise.all([
      tx.get(SETTINGS_REF()),
      Promise.all(courtRefs.map((ref) => tx.get(ref))),
      tx.get(doc(db, "players", playerId)),
    ]);
    const activeCourtCount = settingsSnap.data()?.courtCount || DEFAULT_SETTINGS.courtCount;
    const targetSnap = courtSnaps.find((snap) => snap.id === courtId);
    if (!targetSnap || !playerSnap.exists() || playerSnap.data().status !== "waiting") return;
    const staged = targetSnap.data().staged || [];
    if (staged.length >= 4 || staged.includes(playerId)) return;
    const reservedElsewhere = courtSnaps.some((snap) => {
      const court = snap.data();
      return (
        snap.id !== courtId &&
        (court.number || Number(snap.id)) <= activeCourtCount &&
        (court.staged || []).includes(playerId)
      );
    });
    if (reservedElsewhere) return;
    tx.update(targetSnap.ref, { staged: [...staged, playerId] });
  });
}

// Starts exactly the lineup stored on the court. Reading `staged` inside
// the transaction prevents a slow device or a second organizer from
// starting a stale browser copy that differs from the Up Next card.
export async function startStagedMatch(courtId) {
  ...
  try {
    await runTransaction(db, async (tx) => {
      ...
      tx.update(courtRef, {
        status: "playing",
        playerIds,
        playerNames: playerSnaps.map((snap) => snap.data().name || ""),
        startTime: serverTimestamp(),
        liveScoreA: 0,
        liveScoreB: 0,
        staged: [],
      });
      playerRefs.forEach((ref) => tx.update(ref, { status: "playing", courtId }));
    });
    return { ok: true };
  } catch (error) {
